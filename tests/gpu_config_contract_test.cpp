...
bool Parse(
    ...
    return gpu_avs::ParseCommandLine(static_cast<int>(argv.size()), argv.data(), cfg, error);
}
...
int main(int argc, char** argv) {
    ...
    gpu_avs::WorkloadConfig legacy;
    const std::vector<std::string> legacy_args = {
        "gpu-avs-workload", "--profile", "mixed", "--checksum-interval", "7"
    };
